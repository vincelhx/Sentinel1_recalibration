...
import sys
sys.path.append(
    "/home1/datahome/vlheureu/IFREMER/RETROCALIB/Sentinel1_recalibration")
...
def recalibrate_grd(input_file, save_netcdf=False, overwrite_netcdf=False, save_tiff=True, overwrite_tiff=False):
    ...
    if save_netcdf is False and save_tiff is False:
        logging.error(
            "save_netcdf and save_tiff are both False. Nothing to do.")
        return

    elif save_netcdf is True:
...
def processor_grd():
    ...
    recalibrate_grd(input_file, save_netcdf=args.save_netcdf,
                    overwrite_netcdf=args.overwrite_netcdf, save_tiff=args.overwrite_tiff, overwrite_tiff=args.overwrite_tiff)
